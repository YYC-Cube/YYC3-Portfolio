...
async function screenshotProject(page, project, retries = 1) {
  ...
  try {
    ...
    if (retries > 0) {
      ...
      return screenshotProject(page, project, retries - 1);
    }
...
async function main() {
  ...
  for (const project of projects) {
    const result = await screenshotProject(page, project, 1);
    results.push(result);
  }
